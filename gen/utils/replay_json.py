...
def replay_json(env, json_file):
    ...
    # print("Task: %s" % (traj_data['template']['task_desc']))   # crash
    print("Task: %s" % (traj_data['task_type']))
    ...
    for ll_action in traj_data['plan']['low_actions']:
        ...
        print("HL Action: %s, LL Cmd: %s" % (traj_data['plan']['high_pddl'][hl_action_idx]['discrete_action']['action'],
