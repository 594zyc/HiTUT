...
ALL_OBJECTS_SINGULAR_LEXICALIZED = [
    'alarm clock',
    'apple',
    'apple slice',
    'armchair',
...
OBJECTS_DETECTOR = sorted(list(OBJECTS_DETECTOR))   #length: 73
STATIC_RECEPTACLES = sorted(list(STATIC_RECEPTACLES))   #length: 32
ALL_DETECTOR = sorted(list(set(OBJECTS_DETECTOR) | set(STATIC_RECEPTACLES)))   #length: 105
# print(ALL_DETECTOR)

ACTION_ARGS = ALL_DETECTOR + ['None']

ALL_OBJECTS = ALL_DETECTOR


OBJECTS_TO_WORDS = {ALL_OBJECTS[i]: ALL_OBJECTS_SINGULAR_LEXICALIZED[i] for i in range(len(ALL_OBJECTS))}

OBJECTS_LOWER_TO_UPPER = {obj.lower(): obj for obj in ALL_OBJECTS}
...
UNK = '[UNK]'
SEP = '[SEP]'
SOS = '[SOS]'   # the first input of the high-level decoder
...
ACTION_TO_WORDS = {
    ...
    'OpenObject': 'open',
    'CloseObject': 'close',
    'ToggleObjectOn': 'turn on',
    'ToggleObjectOff': 'turn off',
}
...
def similar(a,b):
    for v in SIMILAR_SET.values():
        if a in v and b in v:
            return True
    return a==b or a.lower() in b.lower() or b.lower() in a.lower()
