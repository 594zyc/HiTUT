...
from gen.utils.video_util import save_frames_before_action, save_frames_after_action
...
class ThorEnv(Controller):
    ...
    def step(self, action, smooth_nav=False, visualize_info=None):
        '''
        overrides ai2thor.controller.Controller.step() for smooth navigation and goal_condition updates
        '''
        events = None
        if visualize_info is not None:
            save_frames_before_action(self.last_event.frame[:, :, ::-1], action, visualize_info)
        if smooth_nav:
        ...
        if visualize_info is not None:
            ...
            save_frames_after_action(events, action, visualize_info)
        return event
    ...
    def smooth_rotate(self, action, render_settings=None):
        ...
        events = []
        for xx in np.arange(.1, 1.0001, .18):   # we use 0.18 in our video generation
            if xx < 1:
                teleport_action = {
                    'action': 'TeleportFull',
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False, visualize_info=None):
        # TODO: parametrized navigation commands

        if "RotateLeft" in action:
            action = dict(action="RotateLeft",
                          forceAction=True)
            event = self.step(action, smooth_nav=smooth_nav, visualize_info=visualize_info)
            ...
            action = dict(action="OpenObject",
                          objectId=object_id,
                          moveMagnitude=1.0)
            event = self.step(action, visualize_info=visualize_info)
    ...
    def va_interact(self, action, interact_mask=None, smooth_nav=True, mask_px_sample=1, debug=False,
        ...
        if debug:
            ...
            img = np.array(self.last_event.frame[:,:,::-1])
            cv2.imshow('full', img)
            cv2.waitKey(1000)
            # if interact_mask is not None:
            #     img = apply_mask(img, interact_mask, color=(0, 1, 0))
            #     img = np.array(img)
            #     xmin, ymin, xmax, ymax = visualize_info['bbox']
            #     cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (0, 255, 0), 1)
            #     cv2.putText(img, visualize_info['class'], (xmin, ymin), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
            ...
            # cv2.putText(img, sg_disp, (5,25), cv2.FONT_HERSHEY_TRIPLEX ,0.7, (225,225,225), 1, cv2.LINE_AA)
            ...
            # cv2.putText(img, a_disp, (5,290), cv2.FONT_HERSHEY_TRIPLEX ,0.7, (225,225,225), 1, cv2.LINE_AA)
