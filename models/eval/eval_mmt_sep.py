...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_path2', type=str, default="exp/something")
    parser.add_argument('--ckpt_name', type=str, default="model_best_seen.pth")
    parser.add_argument('--ckpt_name2', type=str, default="model_best_seen.pth")
    ...
    # setup model2
    args_model2 = argparse.Namespace(**json.load(open(os.path.join(args.eval_path2, 'config.json'), 'r')))
    ckpt_path2 = os.path.join(args.eval_path2, args.ckpt_name2)
    ckpt2 = torch.load(ckpt_path2, map_location=device)
    model2 = MultiModalTransformer(args_model2, alfred_data)
    model2.load_state_dict(ckpt2)   #
    model2.to(model2.device)

    models = {'navi': model2, 'mani': model}
    print('Note that model2 should be a navigation model!')
